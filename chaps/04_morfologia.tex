...
\emph{Número}: certos substantivos são \emph{pluralia tantum} (\gloss{nt.pl.}
...
herdado do indo-europeu (*\emph{-āi} < Dat. *\emph{-āei} e
\gloss{loc.} \emph{*-āi}), o segundo é de época do eslavo comum. O tipo em
-\emph{o}- \gloss{masc.} apresenta ademais uma homofonia entre \gloss{nom.acu.sg.} e
...
particípios e do comparativo (\emph{cf.}~\ref{sec:comparativo} e~\ref{sec:participios}).
...
homofonia entre \gloss{gen.sg.} e \gloss{loc.sg.}, não é mais produtivo em AEE e está em
vias de ser absorvido pelo tipo em -\emph{o}- (os dois tipos têm a mesma
desinência -\emph{ъ} no \gloss{nom.acu.sg.}, o que facilita a passagem de um ao
...
haplologia, \gloss{loc.sg.} -\emph{i} < *-\emph{ēi̯}); essa mesma forma
...
(< -\emph{ēn(s)}, do tipo \glsxtrshort{grego}~ἄρρην ``macho'') no subtipo
\gloss{nom.sg.}
...
O tipo em -\emph{y} (< *-\emph{ū} < *-\emph{uh\textsubscript{2}})
...
é paralelo à extensão do \gloss{gen.acu.} animado para \emph{mati} ``mãe''.


\input{tables/temasemoclusivaes.tex}

O tipo em \emph{-ęt}- < *-\emph{ent}- (\autoref{tab:temasemoclusivaes}) contém
...
indo-europeu do \glsxtrshort{latim} \emph{genus}, \emph{generis},
\glsxtrshort{grego} γένος, γένεος/γένους, do qual conserva a apofonia (o sufixo
tem vocalismo \emph{o} nos
...
\gloss{gen.sg.masc.} \cirilocsex{nova-jego}, \gloss{acu.pl.masc.} \cirilocsex{novy-jẽ},
...
*-\emph{i̯os}-, \emph{cf.} o superlativo \glsxtrlong{grego} -ισ-τος) que aparece
em toda a
...
formado com o sufixo -\emph{ǫšt-} / \emph{-ęšt-}, que continua o sufixo
indo-europeu *-\emph{nt}- recaracterizado em *-\emph{i̯o}
(*\emph{-ont-jo-}) exceto no Nom.sg.masc.; a alternância \emph{ǫ
$\sim$ ę} não resulta diretamente apenas da harmonia
...
tipo em -\emph{ǫs̆t-} no nom.sg.masc, em -\emph{y} para os presentes em
-\emph{e}- e os atemáticos, um tipo em -\emph{ęs̆t-} no nom.sg.masc., em
-\emph{ę} para os presentes em -\emph{i}- e um tipo em -\emph{ǫs̆t}- no
\gloss{nom.sg.} em -\emph{ę} para os presentes em -\emph{je}-. A alternância
entre -\emph{y} e -\emph{ę} no nom.sg.masc. é a mesma que a que se
encontra no \gloss{acu.pl.masc.} dos temas em -\emph{o}- e -\emph{jo}-
...
e o \gloss{acu.sg.}, ainda que o que defina o gênero neutro seja a ausência de
distinção entre \gloss{nom.} e \gloss{acu.} Isso é consequência de o
...
pela distinção entre \gloss{nom.sg.} e \gloss{acu.sg.} para o neutro indeterminado não
...
(\gloss{nom.sg.masc.} \cirilocsex{nesyjI}, \gloss{acu.} \cirilocsex{nesUSijI},
\gloss{gen.} \cirilocsex{nesUSa(a)go}, etc.; \cirilocsex{mol'ijI}, \gloss{acu.}
\gloss{mol'ISijI}, \gloss{gen.} \cirilocsex{mol'ISa(a)go}, etc.).
O \gloss{nom.acu.sg.nt.} é \cirilocsex{nesUSeje}, \cirilocsex{molISeje}.
...
formas átonas de acu. Em emprego preposicional (\cirilocsex{na mẽ}
...
\input{tables/pronpess.tex}

Paradigma do pronome reflexivo ``se'', que existe apenas no singular:
\gloss{acu.} \cirilocsex{sebe} (átono \cirilocsex{sẽ}), \gloss{gen.}
\cirilocsex{sebe}, \gloss{dat.} \cirilocsex{sebE} (átono \cirilocsex{si}),
\gloss{inst.} \cirilocsex{sobojǫ}, \gloss{loc.} \cirilocsex{sebE}.
...
\cirilocsex{vy}) e átona (\gloss{acu.pl.} \cirilocsex{vy}), de onde surge uma
neutralização que permitiu sua extensão ao dual tanto em função do \gloss{nom.}
como do \gloss{acu.} (mas segundo alguns autores \emph{vy} é a forma antiga e
...
oposição entre \gloss{acu.} e \gloss{dat.} contrariamente ao singular, ao que concorre
para a P2 a homofonia entre a forma átona e a forma do nominativo,
tônica.

Para os pronomes pessoais a forma de \gloss{acu.} é sistematicamente idêntica à
forma de \gloss{gen.}, tanto no singular como no plural (mas não no dual): a
identidade entre \gloss{acu.} e \gloss{gen.} é uma marca de animacidade. Dado que
...
o \gloss{gen.-acu.} animado deles seja menos limitado e tenha uma extensão maior
...
\gloss{gen.pl.} e do \gloss{loc. pl.} o que se encontra na flexão determinada do
adjetivo.

\clearpage%
\subsubsection{Demonstrativos}

\input{tables/prondem.tex}
...
nominais, \emph{ě $\sim$ i} (\gloss{Inst.sg.}, \gloss{Gen.dat.inst.loc.pl.},
\gloss{n.acu.du.fem.} e \gloss{nt.}), \emph{o $\sim$ e}
(\gloss{gen.dat.loc.sg.masc.}, \gloss{n.acu.sg.nt.}), \emph{ъ $\sim$ ь}
(\gloss{nom.sg.masc.}).
...
o tipo brando e o tipo duro (\autoref{tab:pronmist}). O paradigma original é de
tipo
...
\cirilocsex{dъva} ``dois''). No \gloss{nom.sg.fem.} e no \gloss{nom.acu.pl.nt.} regularmente
ocorre \cirilocsex{sica}, mas para \emph{vьsь} coexistem duas grafias,
\cirilocsex{vьsě} e \cirilocsex{vsja,vs'a} ou \cirilocsex{vьsa}, que são resultado
da dificuldade de grafar o /s'/ palatalizado (\emph{cf.}~\ref{sec:invcons}). A grafia com
...
notar \emph{ě}. Paralelamente, a grafia \cirilex{vIsõ} em vez de
\cirilocsex{vIsjõ,vьs'ǫ}
para o \gloss{acu.sg.fem.} é regular nos manuscritos búlgaros antigos, mas também
...
dialeto morávio, a forma é \emph{vьšь} (o tratamento *\emph{x}
> \emph{š} para a segunda e a terceira palatalizações é
característico do eslavo ocidental).


\subsubsection{Interrogativo--indefinido}

O interrogativo-indefinido (\autoref{tab:proninterr}) continua as duas funções
herdadas
...
\gloss{acc}. Idêntico ao \gloss{gen.} característico dos animados),
\emph{čь-}/\emph{če-} para o inanimado, com a forma de \gloss{nom.sg.}
\cirilocsex{kъto}, \cirilocsex{Cьto} recaracterizada.

\input{tables/proninterr.tex}
...
(variante \cirilocsex{Cь-} por analogia do nom.acu.); no dativo as formas
...
reforçada pela partícula \cirilocsex{že}, e com as mesmas variantes
(\emph{ničьso} no genitivo no \emph{Missal de Kiev}): em emprego
...
forma antiga do \gloss{nom.sg.} \cirilocsex{kъ} (*\emph{kwos}) não recaracterizada
por \cirilocsex{-to}.

As formas simples \cirilocsex{kъto}, \cirilocsex{čьto} são preservadas em
emprego indefinido apenas em proposições negativas (\emph{ni sъmě kъto
vъprositi jego} ``e ninguém ousou lhe perguntar'' \passageme{Mt2246}),
interrogativas (\emph{imate li čьto}? ``tendes algo?'' \passageme{L 2441}) ou
hipotéticas (\emph{ašte kъto}... ``se alguém...''), assim como em
...
a palatalização no \gloss{nom.pl.masc.} \cirilocsex{ci-ji}), mas não nos casos
oblíquos (\autoref{tab:deminterr}). Como ocorre com o pronome, o AEE reserva
...
kojejęže strasti} ``de mal algum'' \glsxtrshort{Supr}). A partir de
...
O pronome anafórico (\autoref{tab:anafrel}) é baseado em um tema alternante
\emph{i-} (<\emph{\#ji-}) $\sim$ \emph{je-}; nessa
tabela, conserva-se a notação \emph{\#jь} e não \emph{\#i}, que é a
transliteração usual, porque os dois são fonologicamente distintos
(\emph{cf.}~\ref{sec:distrvogais}), em contrapartida, nota-se \emph{\#i-} em
lugar de *\emph{ji-},
...
parênteses na \autoref{tab:anafrel}).
O \gloss{acu.sg.masc.} não tem forma de animado de data antiga:
nos \emph{Evangelhos}, o \gloss{acc}. É regularmente \cirilocsex{jь},
(transliterado \emph{i}) tanto para animados como para inanimados
(\emph{iscěli i} ``ele o cura'' \passageme{L2251}). O alinhamento com os outros
pronomes, contudo, suscitou a substituição de /\cirilocsex{jь} por
\cirilocsex{jego}, forma do genitivo, já nos \emph{Evangelhos}
(\emph{prědajęi ego} ``aquele que o trai'' \passageme{M1455}), portanto, com um
...
\cirilocsex{iZe} (\emph{kamenь jegože běšę privęzali} ``a pedra que eles
firmaram'' \emph{Supr.} 154, 17). No plural, o \gloss{acu.} \cirilocsex{jẽ} começa
a sofrer concorrência do \gloss{gen.-acu.} \cirilocsex{ixъ}, seguindo o modelo do
singular (\emph{vъprosi ję} ``ele os interrogou'' \passageme{Mt2241}
\glsxtrshort{Marianus}, mas \glsxtrshort{Sav} tem \emph{ixъ}).
...
``dele'', \cirilocsex{prědъ n'imi} ``diante deles'', \cirilocsex{o
n'eji} ``a seu sujeito'' (fem.), relativo \cirilocsex{na n'emьZe}
``sobre o qual'', \cirilocsex{na n'ejiZe} ``sobre a qual'',
\cirilocsex{iz n'egoZe} ``do qual''; para o \gloss{acu.sg.masc.} a forma
nasalizada é
...
jemь}, silabificado [vъ|n'e|mь]) e, por conseguinte,
...
uma flexão de dual regular (\gloss{nom.acu.masc.} \cirilocsex{dъva}, \gloss{nt.}
...
Os cardinais de ``cinco'' a ``nove'' (\emph{cf.}~\autoref{anexo:lexico}) são
substantivos, que
...
numerais se constroem com um complemento no genitivo plural
(\emph{cf.}~\ref{5.3.5}).
...
são supletivos (\emph{cf.}~\autoref{anexo:lexico}). Os ordinais de ``onze'' a ``dezenove'' são
formados com o ordinal da unidade e o cardinal da dezena, que permanece
invariável (\emph{vъtoroe na desęte} ``décimo segundo'' no neutro, lit.
