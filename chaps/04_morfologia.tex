...
cisão é uma consequência da harmonia intrassilábica do eslavo comum (cf.
2.2.3.): à medida em que uma vogal posterior tem seu ponto de
...
(tipo mole), no \gloss{acc.masc.pl}., \emph{ons} > *-\emph{uns}
...
e \gloss{nom.acc.dual}). Nos temas em -\emph{sk}-, a forma palatalizada
...
sincretismo do \gloss{dat.loc.sg.} e do \gloss{nom.acc.pl.} é sistemático: o primeiro é
herdado do indo-europeu (*\emph{-āi} \textless{} Dat. *\emph{-āei} e
loc. \emph{*-āi}), o segundo é de época do eslavo comum. O tipo em
-\emph{o}- masc. apresenta ademais uma homofonia entre \gloss{nom.acc.sg.} e
\gloss{gen.pl.} (\emph{-ъ} no tipo duro, -\emph{ь} no tipo mole), entre \gloss{gen.sg.} e
\gloss{nom.acc.du.}, e, unicamente no tipo duro, entre \gloss{acc.pl.masc.} e
\gloss{inst.pl.}
...
\gloss{gen.pl.} (três gêneros), \gloss{acc.pl.} (somente \gloss{masc.} e
\gloss{fem.}, os neutros têm uma desinência específica), \gloss{nom.acc.dual}
...
particípios e do comparativo (cf. 4.2.2. e 4.2.3.).
...
``juiz'', \gloss{acc.sg.} \cirilocsex{bogyn'jõ,bogyn'õ}, \cirilocsex{sõdijõ},
...
desinência -\emph{ъ} no nom.acc.sg., o que facilita a passagem de um ao
...
instaurar uma oposição de gênero entre masc. (-\emph{ovъ}) e fem.nt.
(-\emph{ъ}) e, por outro, restaurar uma oposição entre o
\gloss{nom.acc.sg.masc.} e \gloss{gen.pl.masc.}, homófonos no tipo em -\emph{o}-
...
seu contexto de aparição que é o \gloss{acc.sg.} e ocorre também em uma outra
...
de várias desinências nos dois tipos (\gloss{nom.acc.sg.} -\emph{ь}, \gloss{loc.sg.}
-\emph{i}).
...
(cf. 3.2.5), o que faz com que a distinção entre as duas séries de
...
vocálicas (\gloss{ac.sg.} consonantal *-\emph{m̥} >
esl.~\emph{*-ĭm}, balt. *-\emph{im}, homófono do \gloss{ac.sg.} *-\emph{im} dos temas
...
\gloss{nom.sg.}
\cirilocsex{korẽ}, \gloss{acc.sg.} \cirilocsex{korenI} ``raiz''; quanto ao resto
...
``cidadãos'', \gloss{ac.} -\emph{any}, \gloss{gen.} \emph{-anъ}, \gloss{dat.}
...
Notar-se-á que o \gloss{ac.sg.} tem uma variante \cirilocsex{matere}, com a
...
tomado do tipo em \emph{-jo}- (\gloss{ac.pl.} -\emph{ę}, \gloss{gen.pl.} -\emph{ь},
...
(\gloss{ac.sg.} \emph{svekrъve svojo} ``sua sogra'' \passageme{Mt1035} \glsxtrshort{Zogr}, em
comparação com \emph{svekrъvь svojǫ} em \glsxtrshort{Marianus}): o desenvolvimento
é paralelo à extensão do gen.ac. animado para \emph{mati} ``mãe''.
...
dois paradigmas têm em comum a forma de \gloss{nom.ac.sg.} em -\emph{o}. Por
...
um tema em -\emph{s}-: \gloss{nom.ac.du.} \cirilocsex{oCi}, \cirilocsex{uSi} (que
...
quando ele próprio é substantivado (\emph{cf.} 5.3.4), e uma forma
...
é, etimologicamente, o do pronome relativo (\emph{cf.}~4.3.3.).
...
\gloss{nom.ac.dual} \cirilocsex{niStiji}, \gloss{nom.ac.pl.} \cirilocsex{niStẽjẽ}.
...
apresenta uma homofonia entre \gloss{ac.sg.} e \gloss{inst.sg.} no tipo duro e no mole
...
entre -\emph{ь} (\gloss{sg}) e -\emph{i} (\gloss{pl}) (cf. 3.2.5.).
...
\gloss{gen.sg.masc.} \cirilocsex{nova-jego}, ac.pl.masc. \cirilocsex{novy-jẽ},
\gloss{nom.sg.fem.} \cirilocsex{nova-ja}, \gloss{ac.sg.fem.}
...
flexão exceto no \gloss{nom.acc.sg.} \gloss{masc.} e \gloss{nt.}.
Apenas \gloss{nom.sg.} e \gloss{acc.sg.}
...
após a palatal (cf. 2.2.3.): \cirilocsex{mUnogU} ``numeroso'',
...
para os masculinos e neutros (com exceção do nom.acc.sg.), em
...
\cirilocsex{-ISiji} é formado. Encontra-se por vezes no \gloss{ac.sg.masc.} uma
forma \emph{bol'ISI}, analógica dos particípios presente e passado
cuja flexão é paralela (cf. 4.2.3.). O \gloss{nom.acc.sg.} neutro determinado,
